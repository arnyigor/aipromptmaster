...
interface ChatDao {
    ...
    /**
     * Обновляет идентификатор модели для сообщения.
     *
     * @param messageId Идентификатор сообщения для обновления.
     * @param modelId Идентификатор модели.
     */
    @Query("UPDATE messages SET model_id = :modelId WHERE id = :messageId")
    suspend fun updateMessageModelId(messageId: String, modelId: String)
    ...
    /**
     * Обновляем системный промпт **и** сразу добавляем его как сообщение.
     *
     * Room обернёт весь метод в транзакцию благодаря аннотации @Transaction.
     */
    @Transaction
    suspend fun updateSystemPromptAndAddMessage(
        conversationId: String,
        newPrompt: String
    ) {
        // 1. Обновляем поле systemPrompt в таблице conversations
        updateSystemPrompt(conversationId, newPrompt)

        // 2. Создаём системное сообщение и вставляем его в messages
        val systemMessage = MessageEntity(
            conversationId = conversationId,
            role = ChatRole.SYSTEM.toApiRole(),
            content = newPrompt,
    ...
    /** Получаем конкретное сообщение по id (необязательно, но удобно для транзакций) */
    @Query("SELECT * FROM messages WHERE id = :messageId LIMIT 1")
    suspend fun getMessageById(messageId: String): MessageEntity?

    /**
     * Удаляем сообщение и сразу обновляем метку последнего изменения диалога.
     *
     * Внутри транзакции гарантируется, что либо оба шага выполнены,
     * либо откатываются при ошибке.
     */
    @Transaction
    suspend fun deleteMessageAndRefreshConversation(messageId: String) {
        val msg = getMessageById(messageId)
            ?: throw IllegalArgumentException("No message with id=$messageId")

        // 1. Удаляем сообщение
        deleteMessageById(msg.id)

        // 2. Обновляем timestamp диалога, чтобы список чатов корректно сортировался
