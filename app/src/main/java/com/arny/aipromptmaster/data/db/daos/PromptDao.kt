...
interface PromptDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertPrompts(prompts: List<PromptEntity>): List<Long>

    @Update
    suspend fun updatePrompt(prompt: PromptEntity): Int
    ...
    fun observeById(promptId: String): Flow<PromptEntity?>

    @Query("SELECT * FROM prompts WHERE _id = :promptId")
    suspend fun getById(promptId: String): PromptEntity?

    @Query("SELECT EXISTS(SELECT 1 FROM prompts WHERE LOWER(title) = LOWER(:title) AND (:excludeId IS NULL OR _id <> :excludeId))")
