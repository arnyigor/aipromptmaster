...
interface IChatHistoryRepository {
    ...
    /**
     * Обновляет идентификатор модели для сообщения ассистента.
     * Используется когда API возвращает модель, отличную от запрошенной.
     *
     * @param messageId Уникальный идентификатор сообщения.
     * @param modelId Идентификатор модели, которая сгенерировала ответ.
     */
    ...
    fun updateStreamingBuffer(map: Map<String, String>)

    fun observeStreamingBuffer(): StateFlow<Map<String, String>>

    fun cleanStreamingBuffer()
}